# -*- coding: utf-8 -*-
"""
Created on Tue Mar 12 14:56:17 2019
...
baseURL = 'https://wassermanschultz.house.gov/news/documentquery.aspx?DocumentTypeID=27&Page='

urlsList = []
errorsList = []
for counter in range(1, 73):
    ...
    try:
        ...
        a = soup.find('ul',attrs={"class":"UnorderedNewsList"})
        b = a.findAll('a',attrs={"class":"middleheadline"})
        for link in b:
            url = link.attrs['href']
            url = url.strip()
            pressURL = 'https://wassermanschultz.house.gov/news/'+url
...
text_file = open("debbieWassermanSchultzPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("debbieWassermanSchultzPressErrors.txt", "w")
