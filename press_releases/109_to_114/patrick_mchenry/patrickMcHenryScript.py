# -*- coding: utf-8 -*-
"""
Created on Mon Apr 15 18:41:08 2019
...
urlsList = []
errorsList = []
baseURL = 'https://mchenry.house.gov/news/documentquery.aspx?DocumentTypeID=418&Page='
for counter in range(2, 74):
    ...
    try:
        ...
        for link in a:
            url = link.attrs['href']
            url = url.strip()
            pressURL = 'https://mchenry.house.gov/news/'+url
...
for targetURL in urlsList:
    ...
    try:
        ...
        date = soup.find('div', attrs={"class":"topnewstext"}).get_text()
...
text_file = open("patrickMcHenryPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("patrickMcHenryPressErrors.txt", "w")
