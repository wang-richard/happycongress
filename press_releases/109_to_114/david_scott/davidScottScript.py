# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 16:00:02 2019
...
baseURL = 'https://davidscott.house.gov/news/documentquery.aspx?DocumentTypeID=377&Page='
...
for counter in range(1, 44):
    ...
    try:
        ...
        a = soup.findAll('article',attrs={"class":"newsblocker"})
        for link in a:
            url = link.find('a').attrs['href']
            url = url.strip()
            pressURL = 'https://davidscott.house.gov/news/'+url
...
text_file = open("davidScottPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("davidScottPressErrors.txt", "w")
