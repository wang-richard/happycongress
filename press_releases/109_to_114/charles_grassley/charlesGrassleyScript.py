# -*- coding: utf-8 -*-
"""
Created on Sun Mar 10 20:46:11 2019
...
baseURL = 'https://www.grassley.senate.gov/news/news-releases?page='
urlsList = []
errorsList = []
for counter in range(1, 424):
    ...
    try:
        ...
        a = soup.findAll('span',attrs={"class":"field-content"})
        for link in a:
            url = link.find('a').attrs['href']
            url = url.strip()
            pressURL = 'https://www.grassley.senate.gov'+url
...
for targetURL in urlsList:
    ...
    try:
        ...
        a = soup.find('span', attrs={"class":"date-display-single"})
        date = a.get_text()
        date = date.strip()
        date = parse(date)
        date = str(date)
...
text_file = open("charlesGrassleyPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("charlesGrassleyPressErrors.txt", "w")
