# -*- coding: utf-8 -*-
"""
Created on Mon Mar 18 21:03:37 2019
...
baseURL = 'https://steveking.house.gov/media-center/press-releases?page='
urlsList = []
errorsList = []
for counter in range(1, 143):
    ...
    try:
        ...
        b = a.findAll('h3',attrs={"class":"field-content"})
        for link in b:
            url = link.find('a').attrs['href']
            url = url.strip()
            pressURL = 'https://steveking.house.gov'+url
...
text_file = open("steveKingPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("steveKingPressErrors.txt", "w")
