# -*- coding: utf-8 -*-
"""
Created on Sat Mar 09 19:18:33 2019
...
baseURL = 'https://sires.house.gov/media-center/press-releases?page='
...
for counter in range(1, 77):
    ...
    try:
        ...
        for link in a:
            ...
            pressURL = 'https://sires.house.gov'+url
...
for targetURL in urlsList:
    ...
    try:
        ...
        a = soup.find('div', attrs={"class":"pane-content"})
        date = a.get_text()
        date = date.strip()
        date = parse(date)
        date = str(date)
        ...
        a = soup.find('div',attrs={"class":"field-item even"})
...
text_file = open("albioSiresPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("albioSiresPressErrors.txt", "w")
