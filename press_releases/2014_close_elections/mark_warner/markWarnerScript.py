# -*- coding: utf-8 -*-
"""
Created on Fri Mar 08 16:37:36 2019
...
baseURL = 'https://www.warner.senate.gov/public/index.cfm/pressreleases?page='
urlsList = []
errorsList = []
for counter in range(1, 55):
    ...
    try:
        ...
        a = soup.findAll('h1',attrs={"class":"title"})
        for link in a:
            url = link.find('a').attrs['href']
            url = url.strip()
            pressURL = 'https://www.warner.senate.gov'+url
...
text_file = open("markWarnerPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("markWarnerPressErrors.txt", "w")
