# -*- coding: utf-8 -*-
"""
Created on Sat Mar 09 13:08:41 2019
...
baseURL = 'https://juliabrownley.house.gov/category/press-releases/page/'
urlsList = []
errorsList = []
for counter in range(1, 20):
    ...
    try:
        ...
        a = soup.findAll('article',attrs={"class":"news-item"})
        for link in a:
            b = link.find('h3')
            url = b.find('a').attrs['href']
            url = url.strip()
            pressURL = url
...
for targetURL in urlsList:
    ...
    try:
        ...
        a = soup.find('div',attrs={"class":"page-content clearfix"})
...
text_file = open("juliaBrownleyPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("juliaBrownleyPressErrors.txt", "w")
