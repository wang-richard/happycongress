# -*- coding: utf-8 -*-
"""
Created on Thu Dec 27 21:43:49 2018
@author: Richard
"""
...
baseURL = 'https://paulryan.house.gov/news/documentquery.aspx?DocumentTypeID=1861&Page='
urlsList = []
errorsList = []
for counter in range(1, 62):
    ...
    try:
        ...
        a = soup.findAll('span',attrs={"class":"middlecopy"})
        for link in a:
            url = link.find('a').attrs['href']
            url = url.strip()
            pressURL = 'https://paulryan.house.gov'+url
...
text_file = open("paulRyanPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("paulRyanPressErrors.txt", "w")
