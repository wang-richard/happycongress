...
urlsList = []
errorsList = []
baseURL = 'https://loudermilk.house.gov/news/documentquery.aspx?DocumentTypeID=27&Page='
for counter in range(1, 31):
    ...
    try:
        ...
        for link in a:
            url = link.attrs['href']
            url = url.strip()
            pressURL = 'https://loudermilk.house.gov/news/'+url
...
text_file = open("barryLoudermilkPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("barryLoudermilkPressErrors.txt", "w")
