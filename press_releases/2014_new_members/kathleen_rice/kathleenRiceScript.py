...
urlsList = []
errorsList = []
baseURL = 'https://kathleenrice.house.gov/news/documentquery.aspx?DocumentTypeID=27&Page='
for counter in range(1, 18):
    ...
    try:
        ...
        for link in a:
            url = link.attrs['href']
            url = url.strip()
            pressURL = 'https://kathleenrice.house.gov/newsroom/'+url
...
text_file = open("kathleenRicePress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("kathleenRicePressErrors.txt", "w")
