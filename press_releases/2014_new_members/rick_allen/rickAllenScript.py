...
urlsList = []
errorsList = []
baseURL = 'https://allen.house.gov/news/documentquery.aspx?DocumentTypeID=27&Page='
for counter in range(1, 28):
    ...
    try:
        ...
        for link in a:
            url = link.attrs['href']
            url = url.strip()
            pressURL = 'https://allen.house.gov/news/'+url
...
text_file = open("rickAllenPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("rickAllenPressErrors.txt", "w")
