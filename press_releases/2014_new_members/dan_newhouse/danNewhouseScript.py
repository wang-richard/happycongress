# -*- coding: utf-8 -*-
"""
Created on Sat Apr 27 16:43:55 2019
...
urlsList = []
errorsList = []
baseURL = 'https://newhouse.house.gov/media-center/press-releases?page='
for counter in range(1, 104):
    ...
    try:
        ...
        a = soup.findAll('h3',attrs={"class":"news_title"})
        for link in a:
            url = link.find('a').attrs['href']
            url = url.strip()
            pressURL = 'https://newhouse.house.gov'+url
...
text_file = open("danNewhousePress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("danNewhousePressErrors.txt", "w")
