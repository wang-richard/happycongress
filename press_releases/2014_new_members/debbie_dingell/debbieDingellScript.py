...
urlsList = []
errorsList = []
baseURL = 'https://debbiedingell.house.gov/media-center/press-releases?page='
for counter in range(1, 73):
    ...
    try:
        ...
        for link in a:
            ...
            url = url.strip()
            pressURL = 'https://debbiedingell.house.gov'+url
...
text_file = open("debbieDingellPress.txt", "w")
for press in pressList:
    text_file.write(press.encode('utf-8')+"\n")
text_file.close()

text_file = open("debbieDingellPressErrors.txt", "w")
